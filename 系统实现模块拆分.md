...
- 身份上下文、项目边界和资源访问控制预留。
- 读取数据库事实并向前端投递任务事件。
- 创建任务时只事务登记 `CREATED` Task、`task.requested` 与 Outbox，不在接入层创建分析 Job。
- 创建 `CREATED` Task，并以同一事务写入 `task.requested` 与 Outbox；不在 API 内决定或创建初始 Job。
...
- 消费 `task.requested`，幂等完成输入验证、策略检查、初始状态迁移和首批 Job/Outbox 登记。
- 消费 `task.requested`，完成输入校验、策略检查和管线选择后创建初始 Job 与对应 Outbox。
...
MVP 阶段采用风险分层：每次迭代只阻塞于受影响模块的单元/契约测试和关键安全、事务、并发集成测试；全量故障注入、完整端到端矩阵和镜像验收在里程碑、合并或发布前集中执行。总体分支覆盖率最低 80%，不以覆盖率数字替代主流程验收。
...
4. 核心主流程和高风险失败路径具有测试；跨组件关键协议通过定向集成测试，完整端到端矩阵在里程碑或发布前通过。
