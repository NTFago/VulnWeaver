...
首版采用单个个人账号，不引入成员、角色或 RBAC。账号、登录锁定、密码版本和不透明会话以 PostgreSQL 为事实来源；首次启动只从只读 Secret 引导账号，后续重启不再依赖该 Secret。浏览器使用 `HttpOnly`、`SameSite=Strict` 的会话 Cookie，所有写请求同时校验与会话绑定的 CSRF 令牌。生产环境必须启用 TLS 和 `Secure` Cookie。密码校验使用 Argon2id、有界并发和未知账号虚假校验；改密保留当前会话并撤销其他会话。详细决策见 ADR-017。

## 4. 核心业务流程

API 创建任务时只登记 `CREATED` Task，并在同一事务写入 `task.requested` 与 Outbox；LangGraph 编排服务消费该事件后完成输入验证、策略检查、管线选择和首个 Job 创建。API 不直接创建分析 Job，详细职责边界见 ADR-018。
