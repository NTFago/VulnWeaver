...
| T18 Sandbox Runner 安全基线 | 待验证 | Codex | 已实现版本化 `SandboxRequest`/`SandboxResult`、ToolSpec 精确绑定、无 Shell Docker CLI runtime、只读输入/隔离输出、禁网/非 root/capability drop/资源边界、超时取消、输出 CAS 登记和孤儿回收；Dev Container 中以固定 Alpine 摘要真实验证非 root、禁网、只读根文件系统和进程退出后无容器残留 | 仍需通过 Runner 本身完成真实无害固定命令、输出限制和运行时统计验收；依赖 T04/T09，按 M12/P4 验收 | 2026-09-09 |
