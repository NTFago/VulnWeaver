...
| T37 Markdown 快速路径 CI 修复 | 待验证 | Codex（`ci/markdown-gate-base-fetch`，PR #51 已合并入 `main`） | Q-017 登记；`quality-gate.yml` Markdown gate 在 diff 前浅取 base 提交；修复 PR 全量门禁通过 | 本 PR（markdown-only）CI 实跑验证快速路径为绿后转「已完成」 | 2026-09-11 |
...
| Q-017 | Markdown 快速路径 CI 在浅克隆下执行 `git diff --check <base-sha>` 报 `fatal: bad object`：`python-quality` job 的 checkout 为默认 depth 1，base 提交对象不存在（PR #50 首次实跑即暴露；该检查非必需，红 X 未阻断合并） | 文档类 PR 的门禁信号失真：失败会被常态忽略；若日后把该检查设为必需，所有 docs PR 都会被阻断 | 已修复：`quality-gate.yml` 的 Markdown gate 在 diff 前按 base SHA 做一次单提交浅 fetch（仅快速路径多一次 fetch，代码变更路径不变）；修复本身经 PR 全量门禁验证，快速路径经修复后的 markdown-only PR 实跑验证（见 T37） | 已处理 | Codex |
