...
- **最近一次全量门禁**：`vulnweaver-t30-dev-1` 内 `pytest` 全量通过；435 个测试通过、5 个跳过（均为需 live Runner/Docker 的 opt-in 项），分支覆盖率 82.24%；PostgreSQL/Redis 集成测试通过 `VULNWEAVER_TEST_ADMIN_DATABASE_URL` 和 `VULNWEAVER_TEST_REDIS_URL` 指向 compose 服务名后完整执行。`ruff check .` 与 `pyright` 0 错误 0 警告，`generate_contracts.py --check` 无漂移，TypeScript/Svelte 检查 0 错误。
...
| D-002 | T32 种子语料是否需要在契约中显式建模（独立于目标工件），还是维持“以任务输入工件自身作为种子”的保守默认 | 维持当前默认即可满足验收（种子即目标工件，bundle 预算仍受限）；仅当需要真实覆盖度时才引入独立种子投影，届时新增契约字段并同步消费者 | 项目负责人 | 待确认（2026-09-10） |
...
3. T32 自动投递已接通（`feat/t32-fuzz-auto`，提交 `4ec38a0`）；仅剩真实镜像 E2E。若需独立种子语料，按 D-002 决策后新增契约字段。
