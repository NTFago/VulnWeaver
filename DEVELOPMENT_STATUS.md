...
| Q-007 | analysis-worker 真实 REVIEW 模型探针无法访问外部 endpoint | 不能完成真实模型四语言 E2E；analysis-worker 当前仅加入内部 `analysis-plane`，直接放通公网会突破执行面网络边界 | 按用户授权新增独立 `model-egress` Compose 网络，仅 analysis-worker 接入；endpoint `/models` 与带 `response_format=json_object` 的无害探针均返回 200。后续生产部署仍应将该网络替换为域名白名单代理 | 已解除（开发环境） | Codex |
...
|---|---|---|---|---|
| 2026-09-09 | T20 Proof 持久化检查点 | Dev Container 内 Compose 服务地址执行 `pnpm run check` | Ruff、Pyright 0 错误；pytest 308 passed、总分支覆盖率 81.70%；TypeScript/Svelte 0 错误/0 警告；迁移漂移检查通过 | 尚未接入 Worker/Job 持久化和 API；尚未执行真实 Proof Sandbox 回放、T21/T22 |
...
| 2026-09-09 | REVIEW 模型连通性探针 | 重建 analysis-worker 并在其容器内执行 endpoint `/models` 及 `response_format=json_object` 无害请求 | 通过；模型 endpoint 返回 200，模型 ID 为 `glm-5.3-flash`，响应含 JSON 内容和 token 用量；网络出口由独立 `model-egress` 提供，内部分析网络仍保留 | 真实四语言 REVIEW E2E 及生产域名白名单代理仍待完成 |
| 2026-09-09 | T20 远程 Sandbox 执行接入 | Dev Container `uv lock`；Ruff/Pyright；`pytest tests/proof tests/worker -q` | 通过；analysis-worker 在配置 `SANDBOX_RUNNER_URL` 时自动构造 `SandboxRunnerClient`、`ProofExecutionService` 和 `ProofJobExecutor`；未配置时保持结构化 `proof.executor_unconfigured`；14 passed、18 skipped | 独立 Runner 服务尚未部署，跳过项需在 Compose 集成环境重跑 |
| 2026-09-09 | T18 Sandbox Runner HTTP 服务边界 | Dev Container `uv lock`；Ruff/Pyright；`pytest tests/sandbox_runner -q` | 通过；新增 `/health` 和 `/v1/sandbox/runs`，请求契约校验、可选 bearer token、断连取消均已实现；10 passed、1 skipped | 尚未提供服务镜像、真实 Runner 装配和 Docker Socket 的独立执行面 |
| 2026-09-09 | T18 独立 Runner 服务镜像装配 | Dev Container `docker compose ... config --quiet`；`docker compose ... build sandbox-runner`；Ruff/Pyright | 通过；独立镜像安装 Docker CLI，Compose 仅将 Socket 挂载到 sandbox-runner 并接入 `analysis-plane`；固定 AFL++/CASR profile 由代码装配 | Docker daemon 对服务容器内 bind 路径的可见性仍需改为共享卷协议；未启动服务（缺少 AFL_CASR_IMAGE_DIGEST） |
