...
| T27 逆向分析智能体与混淆特征识别 | 未开始 | 待认领 | — | 壳/混淆特征（控制流平坦化启发式）识别后由模型自主规划脱壳/反编译/解混淆/定点分析步骤，经 T25 循环调度工具，决策与理由入 AgentRun；无模型时降级固定管线（定义见 3.1） | 2026-09-10 |
| T28 解混淆与可读伪代码生成 | 未开始 | 待认领 | — | 修复 angr argv 校验缺陷并端到端启用；控制流平坦化恢复；LLM 伪代码结构归纳/重命名/注释生成可读版本派生工件，原始保留（定义见 3.1） | 2026-09-10 |
...
| T30 关键逻辑标定 | 未开始 | 待认领 | — | 导入表/字符串/tree-sitter 特征 + LLM 确认，标定认证、加解密、注册校验等关键函数；标定结果（位置、调用链、判定依据）入库并在 API/前端展示（定义见 3.1） | 2026-09-10 |
...
| T32 模糊测试接入自动链路与 harness 生成 | 未开始 | 待认领 | — | fuzz Job 纳入任务管线（Finding/审计计划驱动）；LLM 生成 harness 并进入“编译—诊断—修正”有界循环；崩溃簇挂接 Finding 证据（定义见 3.1） | 2026-09-10 |
| T33 前端逆向工作台与人工复核 | 未开始 | 待认领 | — | API 补函数调用边端点；前端调用链点击高亮联动伪代码与判定依据、Agent 决策轨迹页、人工复核提交与标注修正入口（定义见 3.1） | 2026-09-10 |
| T34 报告证据链写实 | 未开始 | 待认领 | — | Markdown/PDF/SARIF 报告包含证据链明细（文件位置/二进制地址、调用路径、Review/Poc 验证结果、证据工件摘要引用）；PDF 补严重等级（定义见 3.1） | 2026-09-10 |
