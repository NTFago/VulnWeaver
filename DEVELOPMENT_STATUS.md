...
| T26 二进制主管线收口（Ghidra 默认可用） | 进行中 | Codex | Sandbox Runner binary-tools 路径已接入 Worker；Runner 自动解析摘要，Worker 通过受保护端点获取摘要；binary-facts 完整事实转换为分析贡献并复用派生工件/PAIR 路径；真实 ELF 回放已成功产出 20 函数、96 指令、33 基本块、31 Xref、18 段伪代码 | 完成数据库 Job 触发后的 Worker→Runner→派生工件→PAIR/API 端到端验收；补齐结果中导入表/字符串等事实映射及资源/挂载配置文档 | 2026-09-10 |
