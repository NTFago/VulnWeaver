...
1. T32 真实 E2E 前置（按序）：①构建 `vulnweaver-afl-casr:fixed`（`apps/fuzz-tool`，需确认含新增 `compile_harness` 路径的新摘要）；②在 sandbox-runner 与 analysis-worker 同时配置该摘要（或依赖 `_resolve_local_image_digest` 本地回退）并设置 `SANDBOX_RUNNER_TOKEN`；③以教学 ELF + 种子运行受限 fuzz，确认崩溃经 `crash-manifest` → `CrashRecord` → `FindingEvidence` 挂接，且 harness 编译诊断在预算内收敛。种子语料维持“即目标工件”默认（D-002 已接受），无契约扩展。
