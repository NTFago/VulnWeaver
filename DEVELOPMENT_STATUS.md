...
| T37 Markdown 快速路径 CI 修复 | 进行中 | Codex（`ci/markdown-gate-move-gate`） | Q-017 登记；Markdown gate 移入具备全量克隆的 `change-scope` job（浅克隆无 base 对象、无凭据不可 fetch） | 修复 PR 全量门禁通过后合并，再以 markdown-only PR 实跑验证快速路径后转「已完成」 | 2026-09-11 |
