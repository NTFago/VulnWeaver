...
| T40 智能体调查式审计与确认链路打通 | 待验证 | Codex（`dev` 分支，worktree `.claude/worktrees/t40-agent-audit`） | ①审计由一次性模型调用改为 `AgentLoop` 驱动的调查循环，新增 8 件只读调查工具（`audit_tools.py`）与 `CodeAuditAgent`（`code_audit.py`），静态扫描器输出降级为线索，候选仍经 PAIR 锚定与既有复核/确认门禁；模型未配置时回落原一次性路径。②修复二进制伪代码读取缺陷（`vulnweaver_pair.pseudocode_text` 成为伪代码形状唯一所有者）。③修复 `AgentLoop` 生成的 `AgentRun.prompt_hash` 缺 `sha256:` 前缀（schema 与 DB CHECK 约束均会拒绝，任何 loop run 都无法落库）。④Policy Engine 不再对自由文本参数做遍历扫描（此前 CWE-22 的 `../` rationale 会导致整个计划被拒）。⑤新增 `derive_established_facts`：类别事实终于可从证据推导；fuzz/proof 结算后对受影响的 Finding 以证据修订 id 重开复核。⑥智能体的 `verification_request`（「这个候选需要动态验证」）不再被丢弃，落为 CONTEXTUAL、weight 0 的溯源证据进入证据链，且结构上不可能满足确认策略。全量门禁 496 passed / 5 skipped（Dev Container 内 PostgreSQL/Redis 集成实跑）、ruff 通过、pyright 0 错误 | 智能体在循环内自主发起动态验证（`symbolic-execute` 经 Sandbox Runner）未实现——缺内联 Runner 注入接线、PAIR 目标地址校验与次数上限，见 ADR-026「未决」；真实模型端到端（源码样本与 ELF 样本）与报告证据链浏览器回归未执行 | 2026-09-11 |
...
| ADR-026 | 审计由 `AgentLoop` 驱动的只读调查循环完成（模型自主选择审什么、可追问、可跟踪调用链），静态扫描器输出降级为线索；候选仍锚定 PAIR 并走既有复核与确认门禁；类别事实改由证据推导；动态验证的证据可重开复核。扩大发现面，不扩大信任面。（2026-09-11，T40） |
...
| 2026-09-11 | T40 动态验证意图落库（`dev`） | `finding-report` 的 `verification_request` 落为 CONTEXTUAL、weight 0 的溯源证据并进入证据链；`derive_established_facts` 与确认门禁均只读 SUPPORTS 关系，故该证据结构上不可能影响确认结论（测试断言 relation/weight/strength 三项）。全量门禁 **496 passed / 5 skipped**、ruff 通过、pyright 0 错误 | 真正的循环内动态执行仍是未决项（ADR-026） |
...
0. **T40 收尾（优先）**：
   - 真实模型验收：在产品设置配置 AUDIT 档模型，分别用源码教学样本与 ELF 教学样本跑任务，确认任务页「智能体运行轨迹」出现**多轮决策与工具调用序列**（而非单条记录）、二进制 Finding 的依据来自真实伪代码（Q-020 修复生效）、报告证据链含调查步骤。
   - 打通自动利用链路：创建/修改项目时开启 `exploit_validation_enabled`，确认得到 confirmed Finding 后 `EXPLOIT` Job 实际投递（Q-021 修复生效）。
   - 智能体内联动态验证：实现 `symbolic-execute`（ToolSpec + 步执行器 + 注入 Runner），目标地址经 `pair.functions_at_address` 校验、地址数与调用次数双上限、证据强度不超过 SUPPORTING 且 `reproducible=false`，并补真实 Runner 回放。
