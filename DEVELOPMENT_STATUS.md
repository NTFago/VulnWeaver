...
| T27 逆向分析智能体与混淆特征识别 | 进行中 | Codex（`feat/t27-reverse-agent`，WIP 已推送） | 已新增控制流扁平化启发式识别器；WIP（`feat/t27-reverse-agent` 分支 `4bbb1c3`，未合并）：orchestrator 新增 `reverse_planning.py`（ReversePlanningAgent 复用 T25 AgentLoop、angr-targeted-analysis 进程内 ToolSpec、DatabaseAgentRunSink）+ 3 项测试；binary executor 新增 `planning_hook`（facts 后、angr 前执行，`_plan_with_agent` 失败结构化降级不中断 Job，`target_addresses` 由规划合并）；analysis-worker 已装配。剩余：跑通规划测试（test_reverse_planning 3 项已写未验证）、执行器钩子集成测试、全量门禁、真实模型验收 | 完成测试与门禁后 PR 合并；接手入口 tests/orchestrator/test_reverse_planning.py | 2026-09-10 |
