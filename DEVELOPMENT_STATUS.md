...
|---|---|---|---|---|---|
| WEB-INTEGRATE 中文审计工作台整合 | 进行中 | Codex 主控/UI | 基于 main@3fcbe1e，整合 frontend-zh@f45b248；主线预设/无资源预算表单/上传过滤/样本折叠/列表伪代码/决策展开全部保留；后端由 Terra high 独立任务处理 | 组件合并、真实阶段/统计/重试语义、智能体与报告区、Linux 门禁和浏览器桌面/窄屏验收；每个逻辑改动提交 | 2026-09-11 |
...
| T41 审计轨迹只读 API 投影 | 已完成 | Codex（`feat/web-audit-observability`，worktree `.worktree/web-audit-backend`） | 新增 `GET /api/tasks/{task_id}/audit-trail` 的明确 DTO：仅用生产端稳定 run-id 规则（含 attempt 时显式标注）关联角色与 Job；无匹配一律 `unknown`，不按模型名推断。T40 调查步骤为 `tool_steps`，仅投影原始 AgentRun 决策中已持久化的调用结果；观察输出未持久化则明确 `not_recorded`。二进制默认聚合为真实 `binary-import` Job 与已登记输出版本，不虚构细分进度 | Linux Dev Container：Ruff 通过；`pytest tests/api/test_api.py -q` 22 passed（1 个既有 Starlette 弃用警告）。只读依赖挂载缺失可执行 Pyright 包，未运行该项 | 2026-09-11 |
...
| Q-024 | Web 曾预期 `POST /api/jobs/{id}/retry`，但后端没有该路由，且失败 Job 的 `job_results` 是以 `job_id` 唯一、不可变的终态事实 | 将既有 FAILED Job 直接改回 queued 会在下一次结算时与原终态结果冲突；创建克隆 Job 又不能让终态 Task 逆转，不能提供表面可点但不改变审计结论的重试按钮 | 已确认不新增单 Job retry。失败记录保留；用户需以同一项目与输入版本创建新 Task 执行“重新审计”。前端已据此移除虚假重试入口 | 已处理 | Codex |
