...
- **最后更新**：2026-09-09 20:47（Asia/Shanghai）
...
| T20 Proof/Exploit 流程 | 进行中 | Codex | 新增版本化 `ProofRequest`、`vulnweaver-proof`、`pocs` 迁移和幂等 `PocRepository`；Finding 事务内同步 `poc_ids` 投影；新增 Finding POC 查询 API；新增 Proof Job Scheduler 和 Worker 执行适配；固定工具身份、脚本工件、镜像摘要和资源预算；未确认 Finding 或未开启项目利用验证时在 Sandbox 前拒绝 exploit | 将 Proof Worker 适配接入 analysis-worker/Sandbox Runner；补充策略拒绝、重放、部分失败和真实无害 Proof Sandbox 验收 | 2026-09-09 |
