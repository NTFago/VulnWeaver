...
| T32 模糊测试接入自动链路与 harness 生成 | 进行中 | Codex（`feat/t32-fuzz-auto`） | 已增加 `JobKind.FUZZ`、幂等 Fuzz Job 调度、结构化 harness 源码生成、编译修正循环、崩溃证据关联器、FuzzJobExecutor 与 analysis-worker 条件化 Sandbox 装配；自动链路尚缺从 Finding/harness 工件构造受限 `FuzzRequest`、崩溃记录持久化为 Evidence 及教学样本 E2E | 完成请求/工件编排与 Evidence 持久化，执行教学样本受限 E2E、全量门禁、台账收口 | 2026-09-10 |
