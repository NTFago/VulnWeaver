            ...
            for diagnostic in result["diagnostics"]:
                for cwe_id in sorted(set(diagnostic["cwe_ids"])):
                    if not _is_cwe_id(cwe_id):
                        continue
                    pair_snapshot = await _pair_snapshot(repositories, diagnostic["location"])
                    ...
                    finding = _finding(finding_id, job, diagnostic, cwe_id)
...
async def _pair_snapshot(repositories: Repositories, location: SourceLocation) -> JsonObject:
    ...
    for function in functions:
        ...
        for item in cast(list[PairFunction], neighborhood["functions"]):
            function_ids.add(item["id"])
        for item in cast(list[PairNode], neighborhood["nodes"]):
            node_ids.add(item["id"])
        for item in cast(list[PairEdge], neighborhood["edges"]):
