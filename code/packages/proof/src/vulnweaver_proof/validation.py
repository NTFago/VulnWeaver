...
_FORBIDDEN_SCRIPT_PATTERNS = {
    "persistence": re.compile(r"(?i)(crontab|systemctl\s+(?:enable|start)|/etc/rc\.local|authorized_keys)"),
...
def validate_generated_script(script: str, *, max_bytes: int = 256 * 1024) -> GeneratedScriptPolicyResult:
    ...
    reasons = tuple(code for code, pattern in _FORBIDDEN_SCRIPT_PATTERNS.items() if pattern.search(script))
