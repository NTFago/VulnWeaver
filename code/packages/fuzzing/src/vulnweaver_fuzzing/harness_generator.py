...
class HarnessModel(Protocol):
    async def complete_structured(self, *, tier: ModelTier, task_id: str, run_id: str,
                                  messages: list[dict[str, str]], output_contract: str,
...
class HarnessGenerator:
    ...
    async def generate(self, *, task_id: str, job_id: str, context: JsonObject) -> str | None:
        response = await self._model.complete_structured(
            tier=ModelTier.PLANNING, task_id=task_id,
            run_id=f"agent-run:harness:{job_id}",
            messages=[{"role": "system", "content": "Return HarnessSource JSON with source (1-65536 chars). Emit source only; no shell, network, persistence or exploit code."},
                      {"role": "user", "content": str(context)}],
            output_contract="HarnessSource", max_output_tokens=8192,
        )
        if response.failure is not None or response.output is None:
            return None
        source = cast(JsonObject, response.output).get("source")
        return source if isinstance(source, str) and source.strip() and len(source) <= 65536 else None
