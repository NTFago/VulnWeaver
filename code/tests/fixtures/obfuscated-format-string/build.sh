...
# dispatcher/state machine); no third-party obfuscator is required, so a
# plain gcc image reproduces the artifact. The sample is distributed as an
# unpacked ELF (closed-source form = binary only; source stays in src/ for
# registration).
#
...
# 3) Behaviour checks inside throwaway containers: a plain key is accepted,
#    and a `%x %x %x` key leaks stack data into local stdout (CWE-134,
#    local echo only).
docker_sh "$CC_IMAGE" '
    set -eu
    chmod +x dist/obfuscated-format-string
    dist/obfuscated-format-string "LICENSE-1234" | grep -q "license accepted"
    leak=$(dist/obfuscated-format-string "%x %x %x")
    echo "$leak" | grep -Eq "[0-9a-f]{4,}"
    echo "[verify] format string leak confirmed (stack hex echoed locally)"
'
