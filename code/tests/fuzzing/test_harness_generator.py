...
async def test_generator_maps_source_and_planning_tier():
    ...
    source = await HarnessGenerator(model).generate(task_id="t", job_id="j", context={"finding": "f"})
...
async def test_generator_failure_or_invalid_output_returns_none():
    assert await HarnessGenerator(Model(failure={"code": "x"})).generate(task_id="t", job_id="j", context={}) is None
    assert await HarnessGenerator(Model({"source": "   "})).generate(task_id="t", job_id="j", context={}) is None
