...
> **当前普通源码任务在完成导入、索引和 PAIR 后，只会按语言运行现有的 Semgrep/cppcheck；如果没有命中规则，就不会产生 Finding，独立复核也不会启动，任务可能直接结束为 `completed/no_findings`。这种做法把“当前少量规则零命中”误当成“完整审计未发现问题”，无法证明关键逻辑、语义发现、动态验证和报告等原设计环节已经执行。建议改为“审计计划驱动的必跑基线 + Finding 驱动的条件深审”：适用的基线检测、覆盖度结算和自动 Markdown 报告不再依赖规则命中，候选复核、Proof、Fuzz 和 Exploit 再根据 Finding、能力、预算与策略按条件运行。只有全部适用的必跑节点完成、覆盖度门禁通过且报告成功生成后，零 Finding 才能结算为 `NO_FINDINGS`。**
