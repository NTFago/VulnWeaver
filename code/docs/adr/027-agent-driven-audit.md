...
- 审计的模型调用成本上升（多轮循环取代单次调用）。`AgentLoopBudget.max_planning_rounds` 默认改为 **None（不设轮次上限）**：调查在模型报告完毕并停止请求步骤、或 token 预算耗尽时结束。仍保留的护栏是每轮步数上限、计划被拒次数上限、连续模型失败上限与模型 token 预算。
...
- 真实模型端到端只覆盖了**智能体本身**：以部署栈中已配置的模型对运行库中一个真实任务（102 个 Python 函数的真实开源项目）直接调用 `CodeAuditAgent.audit()`，智能体自主完成 25 次工具调用、跨 8 轮规划，并报出它自己读到并确认的 CWE-78 定位候选。未覆盖的是**整条管线**（任务 → worker → `semantic_audit` Job → 落库 → 复核 → 报告）与浏览器证据链回归。二进制侧的真实模型验收同样未执行。

### 一条实测教训（已修复）

首次真实端到端显示智能体每轮都在调查、累计 22–30 次工具调用，却始终不产出候选。原因在循环而非模型：`feedback["last_steps"]` 由**累计**步骤列表构造，于是每一轮都把此前所有轮次的步骤重新截断、重新塞进提示词。提示词逐轮膨胀却不含新信息，收敛指令被淹没。改为只携带本轮步骤，并在观察中给出剩余轮数后，同一次审计即正常产出一个定位候选。
