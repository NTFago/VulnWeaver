...
5. GitHub Actions 在 Pull Request 和手工触发时运行，权限仅为读取仓库内容。Pull Request 先按变更范围分类：仅 Markdown 文档变更只执行 `git diff --check`，代码、配置、工作流或其他文件变更执行完整门禁；手工触发始终执行完整门禁。`main` 依靠分支保护只接受已通过该必需检查的 Pull Request，不在合并后对同一变更再次执行。
6. uv、Python 包、pnpm 与 Node.js 包沿用项目国内源策略；GitHub Actions 与基础服务镜像由运行器基础设施获取。

## 后果

- `pnpm run check` 是里程碑、合并与 CI 的完整质量入口；仅 Markdown 文档 PR 不再安装依赖或启动数据库/Redis 服务。
